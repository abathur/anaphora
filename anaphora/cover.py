...
class Dict(Reporter):
	...
	def statistics(self, morfs):
		...
		for code_unit in self.code_units:
			analysis = self.coverage._analyze(code_unit)  # pylint: disable=protected-access
			nums = analysis.numbers
			output[code_unit.name] = {"statements": nums.n_statements, "missing": nums.n_missing, "branches": nums.n_branches, "missing_branches": nums.n_missing_branches, "coverage": nums.pc_covered, "miss": analysis.missing_formatted()}
			total += nums
