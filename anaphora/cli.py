...
class CliConfig(Config):
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(
            description="Runner for anaphora test modules. See: docs",
        ...
        parser.add_argument(
            ...
            "--permissive",
            action="store_true",
            help="count broken tests as simple test failures",
        )
        parser.add_argument(
            "-e", "--earmarks", nargs="?", const=sys.stdin, action=Stream
        )
        save_choices = {
        ...
        parser.add_argument(
            "-s",
            "--save",
            choices=save_choices.keys(),
            help="save SQLite database containing the test results\n"
            + "\n".join(
                [
                    " - {:}\t{:}".format(key, value)
    ...
    def __exit__(self, exception_type, exception_value, tb):
        ...
        if exception_value:
            if isinstance(exception_value, (KeyError, TypeError, ImportError)):
                ...
                out += [
                    "Anaphora encountered the above error which prevented a test from executing. Anaphora can treat this broken test as a simple test failure if run with the -p or --permissive flag."
                ]
                out = "\n".join(out)
            else:
...
def _objgraphs():
    ...
    print(
        objgraph.show_backrefs(
            objgraph.by_type("requirement"), filename="/home/anaphora/backrefs.png"
        )
    )
    print(
        objgraph.show_backrefs(
            objgraph.by_type("Stat"), filename="/home/anaphora/statrefs.png"
        )
    )
    print(
        objgraph.show_backrefs(
            objgraph.by_type("QueryAPI"), filename="/home/anaphora/dbrefs.png"
        )
    )
    print(
        objgraph.show_backrefs(
            objgraph.by_type("Noun"), filename="/home/anaphora/nounrefs.png"
        )
    )
    print(
        objgraph.show_refs(
            objgraph.by_type("Noun"), filename="/home/anaphora/nounrefs2.png"
        )
    )
