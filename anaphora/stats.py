...
class Stat(object):
    ...
    __doc__ = __doc__.format(
        valid_types=", ".join(map(str.lower, VALID_TYPES)),
    ...
    def called(self, name):
        ...
        self.how_to["aggregate"] = {
            key: value.replace("{name}", name)
            for key, value in self.how_to["aggregate"].items()
        }
    ...
    def type(self, column_type):
        ...
        if column_type in self.VALID_TYPES:
            ...
            self.how_to["aggregate"] = {
                key: value.replace("{type}", column_type)
                for key, value in self.how_to["aggregate"].items()
            }
    ...
    def create_sql(self):
        if self.initialized:
            ...
            raise Exception(
                "Attempting to create columns for a stat that wasn't properly initialized."
            )
    ...
    def update_sql(self):
        if self.initialized:
            ...
            raise Exception(
                "Attempting to update columns for a stat that wasn't properly initialized."
            )
    ...
    def sum_sql(self):
        if self.initialized:
            ...
            # you'd have to be doing something willfully odd to get here; cut?
            raise Exception(
                "Attempting to create aggregate columns for a stat that wasn't properly initialized."
            )
