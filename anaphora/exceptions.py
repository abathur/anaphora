...
class TestRunException(AnaphoraException):
    ...
    _context = (
        None
    )  # LATERDO: figure out where to get this (<module> or <function> or whatever; somewhere on one of the various f_code objects proably)
    _cause = None
    _output = None
    _traceback = None
    ...
    def path(self):
        if not self._path and self.__traceback__:
            self._path = (
                self.__traceback__.tb_frame.f_code.co_filename
    ...
    @property
    def issue(self):
        return "{node} has {problem} at {location}".format(
            node=self.node, problem=self.problem, location=self.location
        )
    ...
    def initialize(self):
        ...
        if cause:
            ...
            stack = traceback.format_exception(
                cause.__class__, cause, cause.__traceback__
            )
...
class CommandFailure(TestFailure):
    ...
    @property
    def problem(self):
        return 'exit status {exit_status} for "{node.test}"'.format(
            node=self.node, exit_status=self.exit_status
        )

    @property
    def issue(self):
        return "{problem} in {node.parent} at {location}".format(
            node=self.node, problem=self.problem, location=self.location
        )
