...
class Reporter(object):
    ...
    def format_summary(node, runtime=None, exceptions=None, warnings=None):
        ...
        return "Anaphora run {:} in {:,.4f}s with {:} unignored exceptions{:}.".format(
            "failed" if exceptions else "passed",
            runtime,
            exceptions,
            " and {:} warnings".format(warnings) if warnings else "",
        )
...
class Tree(Reporter):
    ...
    def format_exception(self, exception):
        ...
        return "{message}\n{traceback}{output}\n".format(
            message=exception["e_message"],
            traceback=exception["e_traceback"],
            output=output,
        )
    ...
    def format_node(self, node):
        return "%5.0fms|" % (node["during"] * 1000) + self.mark(node).format(
            "", node["name"], node["description"], pad=node["depth"] * 2
        )

    @staticmethod
    def tracked_stats():
        return (
            Stat(lambda _: _.runtime.checkpoint("during").total_seconds())
            .called("during")
            .type("numeric")
            .aggregate_children(),
            # base test stats
