...
class OurDb(sqlite3.Connection):
	...
	def __init__(self, options):
		...
		super().__init__(dbname, detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
	...
	def add_exception(self, node, exception):
		self.execute("INSERT INTO exceptions (e_class, e_message, e_traceback, e_output, e_line, e_path, e_context, e_terminal, node_id, ignore) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (exception.__class__.__name__, exception.message, exception.traceback.strip(), exception.output, exception.line, exception.path, exception.context, exception.terminal, node.id, node.ignored))
