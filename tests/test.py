...
with app("Anaphora").grammar(["need", "goal", "requirement"]):
	with need("customize tests with before/after hooks"):
		...
		with goal("hook failures are properly tracked") as tracker:
			with requirement("bad before_hook fails properly", before=terminal_hook) as this:
				this.ignore()
				tracker.before = this
			...
			with requirement("before_hook failure is tracked"):
				...
				with requirement("exception captures output"):
					assert tracker.before.exceptions[0].output == ahyuk, "Exception didn't capture output"

			with requirement("bad after_hook fails properly", after=terminal_hook) as this:
				this.ignore()
				tracker.after = this
	...
	with need("queryable testing statistics") as parent:
		with goal("track runtime"):
			with requirement("checkpoint has been set") as funtime:
				...
				assert funtime.runtime.check().total_seconds() > 0, "node has no tracked time checkpoint."

			with requirement("runtime accumulated"):
				import datetime
				assert parent.funtime.runtime["during"].total_seconds() > 0, "node has no tracked runtime."
	...
	with need("integrate other types of tests with anaphora run"):
		...
		for garol in requirement("run another executable as a test").commands(["pep8 anaphora --ignore=W191,E501"]):
			garol.run()
		...
		with goal("run earmarks") as marker:
			with requirement("only run tests that pass a version check") as lint:
				...
				if lint.earmark("<1"):
					pylint.append(lint.command("pylint anaphora --disable=missing-docstring,"))
